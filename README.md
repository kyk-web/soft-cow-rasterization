...
![实验输出图](outputs/optimization.gif)

### 7.2 最终轮廓对比图

下图为本次真实运行的最终对比结果。左侧是由 `cow.obj` 渲染得到的目标剪影，右侧是第 `299/300` 轮优化结果。

![实验输出图](outputs/final_comparison.png)

### 7.3 Loss 曲线

优化过程中总损失与剪影损失的变化如下图所示：

![实验输出图](outputs/loss_curve.png)
...
| Epoch | 图片 |
| --- | --- |
| 0 | ![实验输出图](outputs/epoch_000.png) |
| 75 | ![实验输出图](outputs/epoch_075.png) |
| 150 | ![实验输出图](outputs/epoch_150.png) |
| 299 | ![实验输出图](outputs/epoch_299.png) |
